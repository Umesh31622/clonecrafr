...
//       // backend may return { religious: [...] } or { success:true, religious: [...] } adjust:
//       setReligiousList(relRes.data.religious || relRes.data.religiousList || relRes.data || []);
//     } catch (err) {
//       console.error("Data Fetch Error:", err);
//     }
//   };
...
//         type: tpl.type || "video",
//         status: tpl.status || "active",
//         parentType: tpl.category ? "Category" : tpl.politician ? "Politician" : "Religious",
//         parentId: tpl.category?._id || tpl.politician?._id || tpl.religious?._id || "",
//         file: null,
//         transitionPlacement: tpl.transitionPlacement || "below",
//         profilePosition: tpl.profilePosition || "center",
...
//       if (name === "file") setFilePreview(URL.createObjectURL(f));
//       else if (name === "frameFile") setFramePreview(URL.createObjectURL(f));
//     } else {
//       setForm(prev => ({ ...prev, [name]: value }));
//     }
//   };
...
//       if (editId) {
//         await axios.put(`${BASE_URL}/templates/${editId}`, fd, { headers: { "Content-Type": "multipart/form-data" } });
//       } else {
//         await axios.post(`${BASE_URL}/templates`, fd, { headers: { "Content-Type": "multipart/form-data" } });
//       }
...
//   const parentOptions =
//     form.parentType === "Category" ? categories :
//     form.parentType === "Politician" ? politicians :
//     religiousList;

//   const safeParentOptions = Array.isArray(parentOptions) ? parentOptions : [];

//   return (
//     <Container fluid>
//       <Row className="align-items-center mb-3">
//         <Col><h3>Templates</h3></Col>
//         <Col md={5}>
//           <InputGroup>
...
//             <thead>
//               <tr>
//                 <th>Thumbnail</th><th>Title</th><th>Type</th><th>Parent</th>
//                 <th>Profile Position</th><th>Size</th><th>Shape</th><th>Status</th><th>Actions</th>
//               </tr>
//             </thead>
//             <tbody>
//               {filtered.length ? filtered.map(tpl => (
//                 <tr key={tpl._id}>
//                   <td style={{width:120}}>
//                     {tpl.file ? (tpl.type === "video" ? (
//                       <video src={tpl.file} width="100" height="70" style={{objectFit:"cover"}} />
//                     ) : (
//                       <img src={tpl.file} width="100" height="70" style={{objectFit:"cover", borderRadius:6}} alt="tpl" />
//                     )) : <div style={{width:100,height:70,background:"#eee",display:"flex",alignItems:"center",justifyContent:"center"}}>No file</div>}
//                   </td>
//                   <td>{tpl.title}</td>
//                   <td><Badge bg={tpl.type==="video" ? "primary":"info"}>{tpl.type}</Badge></td>
//                   <td>{tpl.category?.name || tpl.category?.title || tpl.politician?.title || tpl.politician?.name || tpl.religious?.title || "-"}</td>
//                   <td>{tpl.profilePosition || "-"}</td>
//                   <td>{tpl.profileSize || "-"}</td>
//                   <td>{tpl.profileShape || "-"}</td>
//                   <td><Badge bg={tpl.status === "active" ? "success" : "secondary"}>{tpl.status}</Badge></td>
...
//       <Modal show={show} onHide={handleClose} size="lg">
//         <Modal.Header closeButton><Modal.Title>{editId ? "Edit Template" : "Add Template"}</Modal.Title></Modal.Header>
//         <Modal.Body>
//           <Form onSubmit={handleSubmit}>
//             <Row>
//               <Col md={8}><Form.Group className="mb-3"><Form.Label>Title</Form.Label><Form.Control name="title" value={form.title} onChange={handleChange} /></Form.Group></Col>
//               <Col md={4}><Form.Group className="mb-3"><Form.Label>Type</Form.Label><Form.Select name="type" value={form.type} onChange={handleChange}><option value="video">Video</option><option value="graphics">Graphics</option></Form.Select></Form.Group></Col>
//             </Row>

//             <Row>
//               <Col md={6}><Form.Group className="mb-3"><Form.Label>Parent Type</Form.Label><Form.Select name="parentType" value={form.parentType} onChange={handleChange}><option value="Category">Category</option><option value="Politician">Politician</option><option value="Religious">Religious</option></Form.Select></Form.Group></Col>
//               <Col md={6}><Form.Group className="mb-3"><Form.Label>Select {form.parentType}</Form.Label><Form.Select name="parentId" value={form.parentId} onChange={handleChange}><option value="">Choose...</option>{safeParentOptions.map(p => <option key={p._id} value={p._id}>{p.name || p.title || p.title}</option>)}</Form.Select></Form.Group></Col>
//             </Row>

//             <Row>
//               <Col md={6}><Form.Group className="mb-3"><Form.Label>Transition Placement</Form.Label><Form.Select name="transitionPlacement" value={form.transitionPlacement} onChange={handleChange}>{placements.map(p=> <option key={p} value={p}>{p}</option>)}</Form.Select></Form.Group></Col>
//               <Col md={6}><Form.Group className="mb-3"><Form.Label>Orientation</Form.Label><Form.Select name="orientation" value={form.orientation} onChange={handleChange}>{orientations.map(o=> <option key={o} value={o}>{o}</option>)}</Form.Select></Form.Group></Col>
//             </Row>

//             <Row>
//               <Col md={6}><Form.Group className="mb-3"><Form.Label>Profile Position</Form.Label><Form.Select name="profilePosition" value={form.profilePosition} onChange={handleChange}>{profilePositions.map(p=> <option key={p} value={p}>{p}</option>)}</Form.Select></Form.Group></Col>
//               <Col md={6}>{form.type === "video" && <Form.Group className="mb-3"><Form.Label>Transition Type</Form.Label><Form.Select name="transitionType" value={form.transitionType} onChange={handleChange}>{transitionTypes.map(t=> <option key={t} value={t}>{t}</option>)}</Form.Select></Form.Group>}</Col>
//             </Row>

//             <Row>
//               <Col md={6}><Form.Group className="mb-3"><Form.Label>Profile Size</Form.Label><Form.Control name="profileSize" value={form.profileSize} onChange={handleChange} placeholder="e.g. 300x300" /></Form.Group></Col>
//               <Col md={6}><Form.Group className="mb-3"><Form.Label>Profile Shape</Form.Label><Form.Select name="profileShape" value={form.profileShape} onChange={handleChange}>{profileShapes.map(s=> <option key={s} value={s}>{s}</option>)}</Form.Select></Form.Group></Col>
//             </Row>

//             <Form.Group className="mb-3"><Form.Label>Frame Image Upload</Form.Label><Form.Control type="file" name="frameFile" onChange={handleChange} /><div className="mt-2">{framePreview && <img src={framePreview} alt="frame" width="100%" />}</div></Form.Group>

//             <Form.Group className="mb-3"><Form.Label>Template File (image or video)</Form.Label><Form.Control type="file" name="file" onChange={handleChange} /><div className="mt-2">{filePreview && (form.type==="video" ? <video src={filePreview} controls width="100%" /> : <img src={filePreview} width="100%" alt="preview" />)}</div></Form.Group>

//           </Form>
//         </Modal.Body>
//         <Modal.Footer><Button variant="secondary" onClick={handleClose}>Close</Button><Button variant="primary" onClick={handleSubmit} disabled={loading}>{loading ? "Saving..." : editId ? "Update" : "Save"}</Button></Modal.Footer>
//       </Modal>
//     </Container>
//   );
// };
...
const Templates = () => {
  ...
  // fetch all data with pagination + search
  const fetchData = async () => {
    try {
      ...
      const [catRes, polRes, relRes] = await Promise.all([
        axios.get(`${BASE_URL}/categories`),
        axios.get(`${BASE_URL}/politicians`),
        axios.get(`${BASE_URL}/religious`)
      ]);
      ...
      setTemplates(tplRes.data.templates || []);
      setTotalPages(tplRes.data.pages || 1);
        ...
        <Card.Body>
          <Table responsive hover bordered>
            <thead>
              <tr>
                <th>Thumbnail</th><th>Title</th><th>Type</th><th>Parent</th>
                <th>Profile Position</th><th>Size</th><th>Shape</th>
              ...
              {templates.length ? templates.map((tpl) => (
                <tr key={tpl._id}>
                  <td style={{width:120}}>
                    {tpl.file ? (
                      tpl.type === "video" ? (
                        <video src={tpl.file} width="100" height="70" />
                      ) : (
                        <img src={tpl.file} width="100" height="70" style={{objectFit:"cover"}} />
                      )
                    ) : (
                      <div style={{width:100,height:70,background:"#eee"}}>No file</div>
                    )}
                  </td>
                  ...
                  <td>{tpl.profileShape || "-"}</td>
                  <td>
                    <Badge bg={tpl.status === "active" ? "success" : "secondary"}>
                      {tpl.status}
                    </Badge>
                  </td>
                ...
                <tr><td colSpan="9" className="text-center">No templates found</td></tr>
              )}
            </tbody>
          </Table>
        ...
        <Modal.Body>
          <Form onSubmit={handleSubmit}>
            ...
            <Row>
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>Profile Shape</Form.Label>
                  <Form.Select name="profileShape" value={form.profileShape} onChange={handleChange}>
                    {profileShapes.map(s => <option key={s} value={s}>{s}</option>)}
                  </Form.Select>
                </Form.Group>
              </Col>
            </Row>
            ...
            <Form.Group className="mb-3">
              <Form.Label>Frame Image Upload</Form.Label>
              <Form.Control type="file" name="frameFile" onChange={handleChange} />
              <div className="mt-2">{framePreview && (<img src={framePreview} alt="frame" width="100%" />)}</div>
            </Form.Group>
              ...
              <div className="mt-2">
                {filePreview && (
                  form.type === "video"
                    ...
                    : <img src={filePreview} width="100%" alt="preview" />
                )}
              </div>
            </Form.Group>
